...
def fundamentals_to_facts(
    payload: dict[str, Any], symbol: str, lags: dict[str, int]
) -> pd.DataFrame:
                    ...
                    rows.append(
                        {
                            ...
                            "period_end": end,
                            "fiscal_period": ("FY" if freq == "yearly" else "Q")
                            if is_flow
                            else None,
                            "form": f"eodhd-{freq}",
                            "unit": entry.get("currency_symbol") or currency,
                            "available_at": avail,
                            "lag_estimated": estimated,
                            "source": "eodhd",
                            "tag": f"{statement}.{field}",
                            "tag_rank": rank,
                        }
                    )
        ...
        rows.append(
            {
                ...
                "form": "eodhd-earnings",
                "unit": "per_share",
                "available_at": filing_available_at(date.fromisoformat(report[:10])),
                "lag_estimated": False,
                "source": "eodhd",
                "tag": "Earnings.epsActual",
                "tag_rank": 0,
            }
        )
    if not rows:
...
class EodhdProvider:
    ...
    def get_daily_prices(self, symbol: str, start: date, end: date) -> pd.DataFrame:
        rows = self._get(
            f"eod/{symbol}", ttl=self.ttl, **{"from": start.isoformat(), "to": end.isoformat()}
        )
    ...
    def get_exchange_symbols(self, exchange: str, delisted: bool = False) -> pd.DataFrame:
        rows = self._get(
            f"exchange-symbol-list/{exchange}", ttl=24 * 7, **({"delisted": 1} if delisted else {})
        )
